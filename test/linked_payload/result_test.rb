...
module LinkedPayload
  class ResultTest < Minitest::Test
    ...
    def test_ok_and_then_raises_on_non_result
      assert_raises(LinkedPayload::Error::LinkedPayloadError) { ok(1).and_then(&ADD_1) }
    end
    ...
    def test_err_or_else_raises_on_non_result
      assert_raises(LinkedPayload::Error::LinkedPayloadError) { err(1).or_else(&ADD_1) }
    end
    ...
    def test_ok_assign_raises_on_non_result
      assert_raises(LinkedPayload::Error::LinkedPayloadError) { ok({}).assign(:a) { 1 } }
    end

    def test_ok_assign_raises_on_non_hash_value
      assert_raises(LinkedPayload::Error::LinkedPayloadError) { ok(0).assign(:a, &ADD_1) }
    end
