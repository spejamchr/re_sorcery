...
module ReSorcery
  class MaybeTest < Minitest::Test
    ...
    def test_just_and_then_raises_on_non_result
      assert_raises(ReSorcery::Error::ReSorceryError) { just(1).and_then(&ADD_1) }
    end
    ...
    def test_nothing_or_else_raises_on_non_result
      assert_raises(ReSorcery::Error::ReSorceryError) { nothing.or_else { 1 } }
    end
    ...
    def test_just_assign_raises_on_non_result
      assert_raises(ReSorcery::Error::ReSorceryError) { just({}).assign(:a) { 1 } }
    end

    def test_just_assign_raises_on_non_hash_value
      assert_raises(ReSorcery::Error::ReSorceryError) { just(0).assign(:a, &ADD_1) }
    end
